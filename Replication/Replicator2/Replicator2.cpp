...
#include "../Common/SaveToFile.cpp"
#include "../Common/EndsWith.cpp"
#include "../Common/ReadFromFile.cpp"
    ...
    do {
        ...
        else if (result == SOCKET_ERROR)
        {
            ...
            if (iResult > 0) {
                ...
                if (strcmp(recvbuf, "ReadSavedData!@#$%^&&*") == 0) {
                    char* data = ReadFromFile("Replicator1_SavedData.bin");
                    if (data == NULL) {
                        char noData[DEFAULT_BUFLEN] = "NULL";
                        int iResult = send(acceptedSocket, noData, (int)strlen(noData) + 1, 0);
                        if (iResult == SOCKET_ERROR)
                    ...
                    while (d.data[0] != '\0') {
                        ...
                        if (EndsWith(result, suffix)) {
                            size_t newSize = std::strlen(result) - 13;
                            char* shortenedArray = new char[newSize + 1];

                            std::strncpy(shortenedArray, result, newSize);
                            shortenedArray[newSize] = '\0';
                            SaveToFile(shortenedArray, newSize, "Replicator2_SavedData.bin");
                        }
                    ...
                    if (async) {
                        ...
                        if (EndsWith(recvbuf, suffix)) {
                            size_t newSize = iResult - 14;
                            char* shortenedArray = new char[newSize + 1];

                            std::strncpy(shortenedArray, recvbuf, newSize);
                            shortenedArray[newSize] = '\0';
                            SaveToFile(shortenedArray, newSize, "Replicator1_SavedData.bin");
                            data = InitData(shortenedArray);
                        }
...
DWORD WINAPI handleConnectSocket(LPVOID lpParam)
{
    ...
    while (appRunning) {
        ...
        if (iResult > 0) {
            ...
            const std::string suffix = "Save!@#$%^&&*";
            if (EndsWith(&temp[1], suffix)) {
                size_t newSize = std::strlen(&temp[1]) - 13;
                char* shortenedArray = new char[newSize + 1];

                std::strncpy(shortenedArray, &temp[1], newSize);
                shortenedArray[newSize] = '\0';
                SaveToFile(shortenedArray, newSize, "Replicator2_SavedData.bin");
            }
